...
(function () {
  ...
  window.getTileUrl = function () {
    ...
    // #1461 followup: honor customizer's tile-provider pick (#1420 / #1430)
    // when the registry is loaded, in both dark and light themes (the light
    // branch used to always return the hardcoded, unkeyed TILE_LIGHT, which
    // bypassed the Carto ?key= param and showed the "API key required"
    // watermark on any light-themed map using this helper). Falls back to
    // TILE_DARK/TILE_LIGHT if the registry is absent.
    try {
      var getter = isDark ? window.MC_getDarkTileProvider : window.MC_getLightTileProvider;
      if (getter && window.MC_TILE_PROVIDERS) {
        var id = getter();
   ...
   * invertFilter or refUrl/attribution). Returns null when the registry
   * isn't loaded. */
